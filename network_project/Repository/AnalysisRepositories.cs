...
public class DetectionResultRepository : BaseRepository<DetectionResult>, IDetectionResultRepository
{
    ...
    public async Task<Dictionary<string, List<string>>> GetThreatSummaryAsync()
    {
        var latestRun = await _db.DetectionResults
                                 .MaxAsync(r => (DateTime?)r.DetectedAt);
        if (latestRun is null) return [];

        var cutoff = latestRun.Value.AddSeconds(-5);
        ...
        return results
            ...
            .ToDictionary(
                g => g.Key,
                g => g.Select(r => r.Node?.IpAddress ?? "unknown").Distinct().ToList());
    }

    public async Task ClearAllAsync()
    {
        _db.DetectionResults.RemoveRange(_db.DetectionResults);
